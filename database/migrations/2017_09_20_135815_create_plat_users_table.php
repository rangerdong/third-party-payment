...
class CreatePlatUsersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('plat_users', function (Blueprint $table) {
            $table->increments('id');
            //基本信息
            $table->tinyInteger('role')->default(0)->comment('账户类型 0商户 1代理 2商务');
            ...
            //账户类型信息
            $table->string('qq', 15)->default('')->comment('qq号');
            $table->string('contact', 100)->default('')->comment('联系人姓名');
            $table->string('code', 15)->default('')->comment('账户编码');
            $table->string('key', 150)->default('')->comment('密钥');
            $table->integer('upper_id')->default(0)->comment('上级id 0无上级');
            $table->string('trade_pwd')->default('')->comment('交易密码');
            ...
            $table->tinyInteger('recharge_mode')->default(1)->comment('交易模式 0按个人 1按分组');
