...
class CreateRechargeOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('recharge_orders', function (Blueprint $table) {
            ...
            $table->decimal('order_rate', 5, 3)->default(0)->comment('订单费率');
            $table->decimal('order_settle', 16, 6)->comment('实际结算');
            ...
            $table->decimal('proxy_settle', 16, 6)->default(0)->comment('代理结算');
            $table->decimal('business_rate', 5, 3)->default(0)->comment('商务费率');
            $table->decimal('business_settle', 16, 6)->default(0)->comment('商务结算');
            ...
            $table->text('third_return')->nullable()->comment('第三方同步请求返回数据 成功为success');
            ...
            $table->softDeletes();
            $table->timestamps();
            $table->index(['uid', 'app_id', 'merchant_no', 'plat_no']);
