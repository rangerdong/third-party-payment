...
class CreateAssetAccountsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('asset_accounts', function (Blueprint $table) {
            ...
            $table->decimal('total', 14, 4)->default(0)->unsigned()->comment('资金总额');
            ...
            $table->decimal('other_frozen', 14, 4)->default(0)->unsigned()->comment('其余冻结');
            $table->decimal('settled', 14, 4)->default(0)->unsinged()->comment('已结算金额');
            $table->unique('id');
            $table->timestamps();
        });
    }
